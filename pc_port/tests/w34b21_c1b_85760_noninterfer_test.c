/*
 * Natural noninterference certificate for 0x80085760.
 *
 * Uses the accepted controller-idle D_800AFE9C / pad schedule.  Does not
 * force the helper, 0x80095414, 0x8008A72C, a PC, slot state beyond the
 * accepted Table A occupancy, a resolver, or GTE state.  The helper must
 * not run.  Frontiers stay at 0x8008A72C / 0x80071490.
 */
#include <stdio.h>
#include <string.h>
...
/* PsyQ hooks required to link the helper TU; must not be invoked. */
typedef struct { s16 m[3][3]; s32 t[3]; } WmMatrix;
typedef struct { s32 vx, vy, vz, pad; } WmVector;
typedef struct { s16 vx, vy, vz, pad; } WmSvector;
...
void SetRotMatrix(WmMatrix *m) { (void)m; }
void SetTransMatrix(WmMatrix *m) { (void)m; }
...
int main(void)
{
    ...
    check("helper-never-ran", wm_80085760_get_exec_count() == 0u);
