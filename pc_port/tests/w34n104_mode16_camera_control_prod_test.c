/* Focused production certificate for retail 0x800813E8/0x80081470. */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
static void test_initializer(void)
{
    ...
    check(wm_800813E8(SLOT_INDEX) == 1, "init.return");
    ...
    check(read32(POSITION + 0u) == UINT32_C(0x01100000) &&
          ...
          read32(POSITION_COPY + 0u) == UINT32_C(0x01100000) &&
          read32(POSITION_COPY + 4u) == UINT32_C(0x02200000) &&
          read32(POSITION_COPY + 8u) == UINT32_C(0x03300000),
          "init.positions");
...
static void test_latches(void)
{
    ...
    check(wm_80081470(SLOT_INDEX) == 1, "latch1.return");
    check(read16(SLOT + 0x04u) == 0u &&
          read16(SLOT + 0x20u) == 1u &&
          read16(ANGLES + 0u) == UINT16_C(0xFF80) &&
          read16(ANGLES + 2u) == UINT16_C(0xFE00) &&
          read16(ANGLES + 4u) == 0u &&
          read32(SLOT + 0x50u) == UINT32_C(0x00970000) &&
          read32(CAMERA_HEIGHT) == UINT32_C(0x0097F800) &&
          read32(SLOT + 0x54u) == UINT32_C(0xFFF81000) &&
          read32(SLOT + 0x58u) == UINT32_C(0xFFF80100) &&
          read32(SLOT + 0x5Cu) == UINT32_C(0xFFE10000) &&
          read32(SLOT + 0x60u) == UINT32_C(0xFFE00800),
          "latch1.state");

    seed();
    write32(CAMERA_GATE, 1u);
    write16(SLOT + 0x04u, 2u);
    check(wm_80081470(SLOT_INDEX) == 1 &&
          read16(SLOT + 0x04u) == 2u &&
...
static void test_camera_gate(void)
{
    ...
    check(wm_80081470(SLOT_INDEX) == 1 && camera_calls == 1u &&
    ...
    camera_calls = 0u;
    write32(CAMERA_GATE, 1u);
    check(wm_80081470(SLOT_INDEX) == 1 && camera_calls == 0u,
          "camera.gate");
}

static void test_state_one_and_jitter(void)
{
    seed();
    write32(CAMERA_GATE, 1u);
    write16(SLOT + 0x04u, 0u);
    write16(SLOT + 0x20u, 1u);
    write32(SLOT + 0x50u, UINT32_C(0x00630000));
    write32(CAMERA_HEIGHT, UINT32_C(0x00700000));
    write32(SLOT + 0x54u, UINT32_C(0x00010000));
    write32(SLOT + 0x58u, 0u);
    write32(SLOT + 0x5Cu, UINT32_C(0x004B0000));
    write32(SLOT + 0x60u, UINT32_C(0x00400000));
    write32(SLOT + 0x7Cu, UINT32_C(0x00008000));
    write16(CAMERA_INPUT + 2u, 100u);
    write16(CAMERA_INPUT + 0x0Au, 200u);
    random_value = 10;

    check(wm_80081470(SLOT_INDEX) == 1, "state1.return");
    check(read32(SLOT + 0x50u) == UINT32_C(0x00630000) &&
          read32(CAMERA_HEIGHT) == UINT32_C(0x006F9800),
          "state1.primary_clamp");
    check(read32(SLOT + 0x54u) == UINT32_C(0x00010000) &&
          read32(SLOT + 0x58u) == UINT32_C(0x00001000) &&
          read16(ANGLES + 0u) == 1u,
          "state1.roll_approach");
    check(read32(SLOT + 0x5Cu) == UINT32_C(0x004B0000) &&
          read32(SLOT + 0x60u) == UINT32_C(0x00405800) &&
          read16(ANGLES + 2u) == UINT16_C(0x0405),
...
{
    seed();
    write32(SLOT + 0x18u, UINT32_C(0x800813E8));
    write32(SLOT + 0x1Cu, UINT32_C(0x80081470));
    write16(SLOT + 0u, 0u);
    wm_sched_reset();
...
int main(void)
{
    ...
    test_latches();
    test_camera_gate();
    test_state_one_and_jitter();
    test_scheduler_resolution();
    if (failures != 0) {
        fprintf(stderr,
                "W34N104 MODE16 CAMERA CONTROL CERTIFICATE: %d failure(s)\n",
                failures);
        return 1;
    }
    puts("W34N104 MODE16 CAMERA CONTROL CERTIFICATE PASS");
    return 0;
}
