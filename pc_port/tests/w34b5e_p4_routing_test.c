...
int main(void)
{
    printf("=== W34B5E P4 routing test ===\n\n");

    /* ---- Test 1: P4 gate OFF → no execution ---- */
    printf("Test 1: P4 gate OFF\n");
    {
        ...
        /* Don't call wm_80072954_common_tail_p4 — simulating gate OFF. */

        check("P4 entry == 0 (gate OFF)", wm_ctp4_get_entry() == 0);
        check("P4 85fe0_calls == 0", wm_ctp4_get_85fe0_calls() == 0);
        check("P4 alloc_calls == 0", wm_ctp4_get_alloc_calls() == 0);
        check("D_8009D7E8 == 0 (no alloc)", *(u32*)PSX_ADDR(0x8009D7E8) == 0);
        check("D_8009D7EC == 0 (no alloc)", *(u32*)PSX_ADDR(0x8009D7EC) == 0);
    }

    /* ---- Test 2: P4 gate ON → exactly one execution ---- */
    printf("\nTest 2: P4 gate ON, exact route\n");
    {
        memset(g_PsxRam, 0, PSX_RAM_SIZE);
        s_heap_offset = 0;
        s_heap_alloc_count = 0;
        ...
        check("D_8009D7E8 != 0", *(u32*)PSX_ADDR(0x8009D7E8) != 0);
        check("D_8009D7EC != 0", *(u32*)PSX_ADDR(0x8009D7EC) != 0);
    }

    /* ---- Test 3: Repeated P4 entry count ---- */
    printf("\nTest 3: Repeated entry count\n");
    {
        ...
        check("P4 entry == 3 after three calls", wm_ctp4_get_entry() == 3);
        check("P4 85fe0_calls == 3", wm_ctp4_get_85fe0_calls() == 3);
        check("P4 alloc_calls == 6", wm_ctp4_get_alloc_calls() == 6);
    }
    ...
    printf("\nTest 4: Reset clears counters\n");
    {
        ...
        check("entry == 0 after reset", wm_ctp4_get_entry() == 0);
        check("85fe0_calls == 0 after reset", wm_ctp4_get_85fe0_calls() == 0);
        check("alloc_calls == 0 after reset", wm_ctp4_get_alloc_calls() == 0);
        check("last_cut == 0 after reset", wm_ctp4_get_last_cut() == 0);
    }

    /* ---- Test 5: Full P0→P1→P2→P3→P4 chain returns exact cuts ---- */
    printf("\nTest 5: Full chain returns exact cuts\n");
    {
        ...
        check("P4 cut == 0x8007295C", p4 == 0x8007295C);
    }
    ...
    printf("\nTest 6: Forbidden path guards\n");
    {
        ...
        check("forbidden 75228 == 0", wm_ctp4_get_forbidden_75228() == 0);
        check("forbidden scheduler == 0", wm_ctp4_get_forbidden_scheduler() == 0);
        check("forbidden world_loop == 0", wm_ctp4_get_forbidden_world_loop() == 0);
    }
    ...
    printf("\nTest 7: First excluded / next helper ZERO\n");
    {
        ...
        /* Cut must be exactly 0x8007295C (first excluded = next instruction). */
        check("first excluded = 0x8007295C", cut == 0x8007295C);

        /* Forbidden counters must all be zero. */
        check("75228 ZERO", wm_ctp4_get_forbidden_75228() == 0);
        check("scheduler ZERO", wm_ctp4_get_forbidden_scheduler() == 0);
        check("world_loop ZERO", wm_ctp4_get_forbidden_world_loop() == 0);
    }
    ...
    printf("\nTest 8: Prior state preservation\n");
    {
        ...
        *(u32*)PSX_ADDR(0x8009BCDC) = 256;
        ...
        check("P4 ptr1 valid", p4_1 != 0);
        check("P4 ptr2 valid", p4_2 != 0);
        check("P4 ptr1 != P1 ptr1", p4_1 != p1_1);
        check("P4 ptr2 != P1 ptr2", p4_2 != p1_2);
        check("P4 ptr1 != P2 ptr1", p4_1 != p2_1);
        check("P4 ptr2 != P2 ptr2", p4_2 != p2_2);
        check("P4 ptr1 != P3 ptr1", p4_1 != p3_1);
        check("P4 ptr2 != P3 ptr2", p4_2 != p3_2);
        check("P4 ptr1 != P4 ptr2", p4_1 != p4_2);
    }
