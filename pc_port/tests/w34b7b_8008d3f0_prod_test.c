/*
 * W34B7-B production-linked certification test for world callback
 * 0x8008D3F0.
 *
 ...
 * constructed from declarative retail fixtures and the accepted D3F0 write
 * table.  They do not call production helpers or duplicate D3F0 control flow.
...
#include "../src/world_map_callback_8d3f0.c"
...
#define TEST_PRIMARY_1      0x8006F369u
...
#define TEST_OBJECT_TOKEN   0x0D3F04C1u
...
{
    const u32 common[] = {
        TEST_POOL_PTR, TEST_MODE, TEST_EE5C, TEST_EF96, TEST_EF98,
    ...
    const u32 extended[] = {
        TEST_F8E6, TEST_C5AC, TEST_C5B4, TEST_C584,
    ...
    if (address_is_one_of(address, common, sizeof(common) / sizeof(common[0])))
        return true;
    if (fixture->mode >= 1 && fixture->mode <= 3 && address == TEST_F8E6)
        return true;
...
static void build_expected_ram(const Fixture* fixture)
{
    ...
    buffer_store_u32(s_ram_expected, s_slot_addr + TEST_SLOT_58, 15u);
    ...
    heading = buffer_load_u16(s_ram_expected,
                              s_slot_addr + TEST_SLOT_HEADING);
    buffer_store_u16(s_ram_expected, TEST_EF96, (u16)oracle_sra12(x));
    buffer_store_u16(s_ram_expected, TEST_EF98, (u16)oracle_sra12(z));
    buffer_store_u16(s_ram_expected, TEST_EE5C, heading);
}
...
static void prepare_fixture(const Fixture* fixture)
{
    ...
    raw_store_u16(TEST_EE5C, fixture->initial_heading);
    raw_store_u8(TEST_F8E5, 0xA5u);
    raw_store_u8(TEST_F8E6, fixture->f8e6);
    raw_store_u8(TEST_F8E7, 0xE7u);
    raw_store_u8(TEST_PRIMARY_1, 0x52u);
...
static void verify_access_matrix(const Fixture* fixture)
{
    ...
    check_case(fixture->name, "F8E6 read gate exact",
               trace_address_count(TEST_F8E6) == expected_flag_reads);
    check_case(fixture->name, "F8E5/F8E7 direct access zero",
    ...
    check_case(fixture->name, "primary/D940/BDF8 direct access zero",
               trace_address_count(TEST_PRIMARY_1) == 0 &&
    ...
    check_case(fixture->name, "heading global access exact",
               trace_address_count(TEST_EE5C) == 2 &&
               trace_address_count(TEST_EE5A) == 0 &&
               trace_address_count(TEST_EE5E) == 0);
    check_case(fixture->name, "position destination access exact",
               trace_address_count(TEST_EF96) == 1 &&
               trace_address_count(TEST_EF98) == 1 &&
               trace_address_count(TEST_EF90) == 0 &&
               trace_address_count(TEST_EF92) == 0 &&
               trace_address_count(TEST_EF9C) == 0 &&
               trace_address_count(TEST_EF9E) == 0 &&
               trace_address_count(TEST_BAD7_EF96) == 0 &&
               trace_address_count(TEST_BAD7_EF98) == 0);
...
static void verify_extended_order(const Fixture* fixture)
{
    ...
    size_t start = trace_index(TRACE_ADDRESS, TEST_F8E6, 1);
    size_t index;
...
static void verify_fixture_mutants(const Fixture* fixture, s32 actual_return)
{
    ...
    report_mutant("C364 wrong channel zero/two",
                  s_c364_channel == 1);
    report_mutant("EE5A/EE5E substituted for EE5C",
                  trace_address_count(TEST_EE5C) == 2 &&
                  trace_address_count(TEST_EE5A) == 0 &&
                  trace_address_count(TEST_EE5E) == 0);
    report_mutant("EF90/EF92, EF9C/EF9E, or 0x8007xxxx destination",
                  trace_address_count(TEST_EF96) == 1 &&
                  trace_address_count(TEST_EF98) == 1);
    report_mutant("slot+58 sibling constant 30/31",
                  raw_load_u32(s_slot_addr + TEST_SLOT_58) == 15u);
    ...
    if (fixture->mode >= 1 && fixture->mode <= 3 && fixture->f8e6 != 1u) {
        report_mutant("F8E6 nonzero instead of exact equality-to-one",
                      s_terrain_calls == 0 &&
                      trace_address_count(TEST_C5AC) == 0);
        report_mutant("terrain called when F8E6 is not one",
                      s_terrain_calls == 0);
    }
    if (fixture->route == ROUTE_EXTENDED) {
...
static RunResult run_fixture(const Fixture* fixture)
{
    ...
    result.return_value = wm_8008D3F0(TEST_SLOT_INDEX);
    s_trace_armed = false;
    ...
    check_case(fixture->name, "C364 receives literal channel one",
               s_c364_channel == 1);
    ...
    check_case(fixture->name, "slot+58 exact word 15",
               raw_load_u32(s_slot_addr + TEST_SLOT_58) == 15u);
    check_case(fixture->name, "slot+5C sign-extends initial EE5C",
               raw_load_u32(s_slot_addr + TEST_SLOT_SIGNED) ==
                   oracle_sign_extend_u16(fixture->initial_heading));
    check_case(fixture->name, "common EF96 exact SRA12 low16",
               raw_load_u16(TEST_EF96) == (u16)oracle_sra12(expected_x));
    check_case(fixture->name, "common EF98 exact SRA12 low16",
               raw_load_u16(TEST_EF98) == (u16)oracle_sra12(expected_z));
    check_case(fixture->name, "common EE5C exact current heading",
               raw_load_u16(TEST_EE5C) ==
    ...
    result.flag_reads = trace_address_count(TEST_F8E6);
    return result;
}
...
static const Fixture s_natural_fixture = {
    "natural-slot5", 1, 0u, ROUTE_COMMON, 1,
    0u, 1u, 0u, 0u, 0u, TEST_OBJECT_TOKEN,
...
typedef struct ModeClassCase {
    s32 input;
    enum wm_d3f0_mode_family expected;
} ModeClassCase;
...
static const ModeClassCase s_mode_class_cases[] = {
    { INT32_MIN, WM_D3F0_MODE_NONPOSITIVE },
    { -1,        WM_D3F0_MODE_NONPOSITIVE },
    { 0,         WM_D3F0_MODE_NONPOSITIVE },
    { 1,         WM_D3F0_MODE_1_TO_3 },
    { 3,         WM_D3F0_MODE_1_TO_3 },
    { 4,         WM_D3F0_MODE_4_TO_7 },
    { 7,         WM_D3F0_MODE_4_TO_7 },
    { 8,         WM_D3F0_MODE_8_OR_GREATER },
    { INT32_MAX, WM_D3F0_MODE_8_OR_GREATER },
};

static enum wm_d3f0_mode_family mutant_classify_mode_unsigned(s32 mode)
{
...
static void run_mode_classifier_tests(void)
{
    ...
    for (index = 0u;
         ...
         index++) {
        enum wm_d3f0_mode_family actual =
            wm_d3f0_classify_mode(s_mode_class_cases[index].input);
        enum wm_d3f0_mode_family mutant =
...
static void run_full_width_sra_tests(void)
{
    ...
    for (index = 0u; index < sizeof(s_sra_cases) / sizeof(s_sra_cases[0]);
         index++) {
        u32 actual = wm_d3f0_sra12(s_sra_cases[index].input);
...
int main(void)
{
    ...
    check_case("natural-slot5", "natural return is one",
               natural.return_value == 1);
    check_case("natural-slot5", "natural direct terrain skipped",
               natural.terrain_calls == 0);
    check_case("natural-slot5", "natural object remains published",
               raw_load_u32(s_slot_addr + TEST_SLOT_OBJECT) ==
                   TEST_OBJECT_TOKEN);
    check_case("natural-slot5", "natural mode1 reads F8E6 once",
