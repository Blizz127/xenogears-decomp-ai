...
BUILD_DIR="${TEMP1_WRAPPER_BUILD_DIR:-$ROOT/pc_port/build_native/temp1_wrapper_alloc_233a4}"
CC="${CC:-gcc}"
mkdir -p "$BUILD_DIR"
cd "$ROOT"
...
echo "retail func_800233A4 slice sha256: $(dd if=disc/SLUS_006.64 bs=1 \
    skip=$((0x800 + 0x800233A4 - 0x80010000)) count=$((0x9c)) status=none \
...
build_and_run() {
    ...
    "$CC" "${BASE[@]}" "${INC[@]}" -w -fno-inline -fno-builtin "$@" \
        ...
        -c src/slus_006.64/system/temp1.c -o "$BUILD_DIR/$name.libc.o"
    "$CC" "${BASE[@]}" "${INC[@]}" -Wall -Wextra -Werror "$@" \
        -c pc_port/tests/temp1_wrapper_alloc_233a4_retail_test.c \
        -o "$BUILD_DIR/$name.test.o"
    objcopy --weaken-symbol=func_80022DF4 \
            --weaken-symbol=func_80022EB8 \
            --weaken-symbol=func_80023804 \
            "$BUILD_DIR/$name.libc.o"
...
rg -q '^TEMP1 WRAPPER ALLOC certificate PASS checks=[0-9]+$' "$BUILD_DIR/O0.stdout"
