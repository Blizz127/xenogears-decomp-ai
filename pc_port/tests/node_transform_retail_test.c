...
extern void func_801E7094(u8*,u8*,s32,s32,s32,s32) __attribute__((weak));
...
int main(void){
 if(!func_801E7094){fputs("NODE TRANSFORM FAIL missing native body\n",stderr);return 1;}
 ...
 for(unsigned flags=0;flags<256;++flags)for(unsigned count=1;count<=5;++count)for(unsigned selected=0;selected<count;++selected)for(unsigned shape=0;shape<3;++shape)for(unsigned seed=0;seed<4;++seed){
  ...
  if(PcPortMipsRun(&cpu,0x801e7094,0xfffffffcu,10000)!=PC_PORT_MIPS_HALTED){fprintf(stderr,"NODE TRANSFORM FAIL oracle %s\n",cpu.error);return 1;}
  memcpy(expected,fixture,sizeof(fixture));memcpy(fixture,initial,sizeof(fixture));func_801E7094(obj,node,mode,args[0],args[1],args[2]);
  if(memcmp(fixture,expected,sizeof(fixture))){fprintf(stderr,"NODE TRANSFORM FAIL flags=%u count=%u selected=%u shape=%u seed=%u\n",flags,count,selected,shape,seed);return 1;}++cases;
 }
 printf("NODE TRANSFORM PASS %u complete-memory comparisons, real recursive retail calls\n",cases);
}
