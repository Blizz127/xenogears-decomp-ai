/* Focused production certificate for retail 0x8007B604/0x8007B798. */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
static u32 pattern(unsigned call_index, unsigned word_index)
{
    return UINT32_C(0xB0000000) + (u32)call_index * UINT32_C(0x01000000) +
           (u32)word_index;
}
...
static int region_is_pattern(u32 address, unsigned call_index)
{
    ...
    for (i = 0u; i < 8u; i++)
        if (read32(address + (u32)i * 4u) != pattern(call_index, i))
            return 0;
    return 1;
}
...
static void seed(void)
{
    ...
    for (i = 0u; i < 8u; i++)
        write32(BASE_MATRIX + (u32)i * 4u,
                UINT32_C(0x22000000) + (u32)i);
    write32(CONTEXT + 0x238u, DESCRIPTOR_A);
    write32(CONTEXT + 0x240u, SOURCE_A);
    write32(CONTEXT + 0x28Cu, DESCRIPTOR_B);
    write32(CONTEXT + 0x294u, SOURCE_B);
    write16(DESCRIPTOR_A + 4u, 7u);
    write16(DESCRIPTOR_B + 4u, 9u);
    write16(SOURCE_A + 4u, 44u);
    write16(SOURCE_B + 4u, 66u);
    primitive_count = 0u;
    scale_count = 0u;
}
...
static void check_base(unsigned call_index, const char *name)
{
    unsigned i;

    for (i = 0u; i < 8u; i++)
        if (scale_input[call_index][i] != UINT32_C(0x22000000) + (u32)i) {
            check(0, name);
            return;
        }
    check(1, name);
...
static void test_initializer(void)
{
    ...
    check(wm_8007B604(SLOT_INDEX) == 3, "init.return");
    check(primitive_count == 2u &&
          primitive_calls[0].owner == CONTEXT + 0x1F8u &&
          primitive_calls[0].source == SOURCE_A &&
          primitive_calls[0].count == 7u &&
          primitive_calls[1].owner == CONTEXT + 0x24Cu &&
          primitive_calls[1].source == SOURCE_B &&
          primitive_calls[1].count == 9u,
          "init.primitive_calls");
    check(primitive_count == 2u && primitive_calls[0].count == 7u &&
          primitive_calls[1].count == 9u,
    ...
    check_base(0u, "init.base_matrix");
    check(region_is_pattern(CONTEXT + 0x2C0u, 1u) &&
          region_is_pattern(CONTEXT + 0x314u, 1u),
          "init.matrix_destinations");
    check(read16(CONTEXT + 0x2A0u) == 1u &&
          read16(CONTEXT + 0x2F4u) == 1u,
          "init.owner_states");
}
...
static void test_update(void)
{
    seed();
    write16(SLOT + 4u, 1u);
    write16(CONTEXT + 0x1F8u, UINT16_C(0x7777));
    write16(CONTEXT + 0x24Cu, UINT16_C(0x8888));
    write32(RESET_POS + 0u, UINT32_C(0xFFF00000));
    write32(RESET_POS + 8u, UINT32_C(0x00200000));
    write32(SLOT + 0x50u, 1664u);
    write32(SLOT + 0x54u, 91u);

    check(wm_8007B798(SLOT_INDEX) == 1, "update.return");
    check(read16(SLOT + 4u) == 0u &&
          read16(CONTEXT + 0x1F8u) == 0u &&
          read16(CONTEXT + 0x24Cu) == 0u,
          "update.latch_clear");
    check(read32(CONTEXT + 0x200u) == UINT32_C(0xFFFFFF00) &&
          read32(CONTEXT + 0x254u) == UINT32_C(0xFFFFFF00) &&
          read32(CONTEXT + 0x204u) == UINT32_C(0xFFFFFFC0) &&
          read32(CONTEXT + 0x258u) == UINT32_C(0xFFFFFFC0) &&
          read32(CONTEXT + 0x208u) == 512u &&
          read32(CONTEXT + 0x25Cu) == 512u,
          "update.positions");
    check(read32(SLOT + 0x50u) == 2048u &&
          read32(SLOT + 0x54u) == 91u,
          "update.stagger_threshold");
    check(scale_count == 2u &&
          scale_xyz[0][0] == 2048 && scale_xyz[0][1] == 4096 &&
          scale_xyz[0][2] == 2048 &&
          scale_xyz[1][0] == 91 && scale_xyz[1][1] == 4096 &&
          scale_xyz[1][2] == 91,
          "update.scale_vectors");
    check_base(0u, "update.primary_base_matrix");
    check_base(1u, "update.secondary_base_matrix");
    check(region_is_pattern(CONTEXT + 0x218u, 1u) &&
          region_is_pattern(CONTEXT + 0x26Cu, 2u),
...
static void test_phase_clamps(void)
{
    ...
    check(wm_8007B798(SLOT_INDEX) == 1 &&
...
{
    seed();
    write32(SLOT + 0x18u, UINT32_C(0x8007B604));
    write32(SLOT + 0x1Cu, UINT32_C(0x8007B798));
    write16(SLOT + 0u, 0u);
    wm_sched_reset();
...
int main(void)
{
    ...
    if (failures != 0) {
        fprintf(stderr,
                "W34N74 MODE14 SCALED OBJECTS B CERTIFICATE: %d failure(s)\n",
                failures);
        return 1;
    }
    puts("W34N74 MODE14 SCALED OBJECTS B CERTIFICATE PASS");
    return 0;
}
