...
static int memory(u32 a,unsigned w,u32*v,int write){
 u8*p=NULL;
 if(w==4&&a==0x80101018){if(write)return -1;*v=0x80114000;return 0;}
 ...
 if(a>=0x80114000&&(uint64_t)a+w<=0x80115400)p=weapons+a-0x80114000;
 else if(a>=0x801010b8&&(uint64_t)a+w<=0x801010ca)p=(u8*)&resources.unkB8+a-0x801010b8;
 else if(a>=0x80110000&&(uint64_t)a+w<=0x80111c00)p=items+a-0x80110000;
...
static void prepare(unsigned seed,unsigned id,unsigned slot){
 init(seed,0,0,slot);pattern(items,sizeof items,seed+7);pattern(weapons,sizeof weapons,seed+9);
 for(unsigned i=0;i<256;i++)D_801E9808[i]=(seed+i)%MAX_GAME_CHARACTERS;
 u32 ptr=(u32)(uintptr_t)items;memcpy(resources.unk8+0xc,&ptr,4);ptr=(u32)(uintptr_t)weapons;memcpy(resources.unk8+0x10,&ptr,4);
 for(unsigned i=0;i<256;i++){
  items[i*28+0x15]=seed<256?seed:items[i*28+0x15];
  weapons[i*20+17]=seed&1?100:99;
 }
 unsigned ch=seed%MAX_GAME_CHARACTERS;manager.currentCharacterIDs[slot]=ch;stateStorage[0x30c+ch*0xa4]=id;
}
int main(void){
 FILE*f=fopen("disc/menu.bin","rb");if(!f)return 2;size_t n=fread(ram+0x1c5000,1,0x3b000,f);fclose(f);if(n<0x1f998)return 2;
 unsigned cases=0;
 for(unsigned seed=0;seed<320;seed++)for(unsigned id=0;id<MAX_GAME_GEARS;id++)for(unsigned slot=0;slot<3;slot++){
  prepare(seed,id,slot);PcPortMipsBus bus={.read=rd,.write=wr};PcPortMipsCpu c;PcPortMipsCpuInit(&c,&bus);c.gpr[4]=0xffffff00|slot;c.gpr[29]=0x801ff000;c.gpr[31]=0xfffffffc;
  if(PcPortMipsRun(&c,0x801dfe2c,0xfffffffc,15000)){fprintf(stderr,"FAIL retail %s\n",c.error);return 1;}
  u8 expected[sizeof stateStorage],ei[sizeof items],ew[sizeof weapons],em[256];memcpy(expected,stateStorage,sizeof expected);memcpy(ei,items,sizeof ei);memcpy(ew,weapons,sizeof ew);memcpy(em,D_801E9808,256);MenuUnk6 er=resources;MenuManager en=manager;SystemMenu eu=menu;
  prepare(seed,id,slot);func_801DFE2C(slot);
  if(memcmp(expected,stateStorage,sizeof expected)||memcmp(ei,items,sizeof ei)||memcmp(ew,weapons,sizeof ew)||memcmp(em,D_801E9808,256)||memcmp(&er,&resources,sizeof er)||memcmp(&en,&manager,sizeof en)||memcmp(&eu,&menu,sizeof eu)){
   fprintf(stderr,"FAIL gear integration seed=%u id=%u slot=%u\n",seed,id,slot);return 1;
  }cases++;
 }
 printf("GEAR INTEGRATION PASS cases=%u\n",cases);return 0;
}
