...
int main(void)
{
    printf("=== W34B5C P2 routing test ===\n\n");

    /* ---- Test 1: P2 gate OFF → no execution ---- */
    printf("Test 1: P2 gate OFF\n");
    {
        ...
        /* Don't call wm_80072944_common_tail_p2 — simulating gate OFF. */

        check("P2 entry == 0 (gate OFF)", wm_ctp2_get_entry() == 0);
        check("P2 8901c_calls == 0", wm_ctp2_get_8901c_calls() == 0);
        check("P2 alloc_calls == 0", wm_ctp2_get_alloc_calls() == 0);
        check("D_8009BE1C == 0 (no alloc)", *(u32*)PSX_ADDR(0x8009BE1C) == 0);
        check("D_8009BE20 == 0 (no alloc)", *(u32*)PSX_ADDR(0x8009BE20) == 0);
    }

    /* ---- Test 2: P2 gate ON → exactly one execution ---- */
    printf("\nTest 2: P2 gate ON, exact route\n");
    {
        memset(g_PsxRam, 0, PSX_RAM_SIZE);
        s_heap_offset = 0;
        s_heap_alloc_count = 0;
        ...
        check("D_8009BE1C != 0", *(u32*)PSX_ADDR(0x8009BE1C) != 0);
        check("D_8009BE20 != 0", *(u32*)PSX_ADDR(0x8009BE20) != 0);
    }

    /* ---- Test 3: Repeated P2 entry count ---- */
    printf("\nTest 3: Repeated entry count\n");
    {
        ...
        check("P2 entry == 3 after three calls", wm_ctp2_get_entry() == 3);
        check("P2 8901c_calls == 3", wm_ctp2_get_8901c_calls() == 3);
        check("P2 alloc_calls == 6", wm_ctp2_get_alloc_calls() == 6);
    }
    ...
    printf("\nTest 4: Reset clears counters\n");
    {
        ...
        check("entry == 0 after reset", wm_ctp2_get_entry() == 0);
        check("8901c_calls == 0 after reset", wm_ctp2_get_8901c_calls() == 0);
        check("alloc_calls == 0 after reset", wm_ctp2_get_alloc_calls() == 0);
        check("last_cut == 0 after reset", wm_ctp2_get_last_cut() == 0);
    }

    /* ---- Test 5: Full P0→P1→P2 chain returns exact cuts ---- */
    ...
    printf("\nTest 6: Forbidden path guards\n");
    {
        ...
        check("forbidden 865a0 == 0", wm_ctp2_get_forbidden_865a0() == 0);
        check("forbidden 85fe0 == 0", wm_ctp2_get_forbidden_85fe0() == 0);
        check("forbidden scheduler == 0", wm_ctp2_get_forbidden_scheduler() == 0);
        check("forbidden world_loop == 0", wm_ctp2_get_forbidden_world_loop() == 0);
    }
    ...
    printf("\nTest 7: First excluded / next helper ZERO\n");
    {
        ...
        /* Cut must be exactly 0x8007294C (first excluded = 0x800865A0). */
        check("first excluded = 0x8007294C", cut == 0x8007294C);

        /* Forbidden counters must all be zero. */
        check("865a0 ZERO", wm_ctp2_get_forbidden_865a0() == 0);
        check("85fe0 ZERO", wm_ctp2_get_forbidden_85fe0() == 0);
        check("scheduler ZERO", wm_ctp2_get_forbidden_scheduler() == 0);
        check("world_loop ZERO", wm_ctp2_get_forbidden_world_loop() == 0);
    }
    ...
    printf("\nTest 8: Prior state preservation\n");
    {
        ...
        *(u32*)PSX_ADDR(0x8009BCDC) = 256;
        ...
        /* P2 allocations valid and distinct from P1. */
        ...
        check("P2 ptr1 valid", p2_1 != 0);
        check("P2 ptr2 valid", p2_2 != 0);
        check("P2 ptr1 != P1 ptr1", p2_1 != p1_1);
        check("P2 ptr2 != P1 ptr2", p2_2 != p1_2);
        check("P2 ptr1 != P2 ptr2", p2_1 != p2_2);
    }
