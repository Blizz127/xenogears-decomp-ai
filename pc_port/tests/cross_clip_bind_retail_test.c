...
#endif
/* Link the unchanged binders against the fixture's strong recording VM.
 * This test-only weak declaration also prevents VM body inlining. */
void func_801E39F0(u8*,void*,s32,s32,s32) __attribute__((weak,noinline));
#include OBJECT_OVERLAY_SOURCE
#else
#include "battle_mips_adapter.h"
extern void func_801E8394(u8*,s32,s32,s32) __attribute__((weak));
extern void func_801E35D0(u8*,u8*,void*,s32) __attribute__((weak));
extern u32 D_801E8670[10];
extern u8 D_801E86A8[8];
extern s16 D_801E863C, D_801E86B0;
...
int main(void){
 if(!func_801E8394){fputs("CROSS CLIP FAIL missing native entry\n",stderr);return 1;}
 ...
 for(unsigned caseIndex=0;caseIndex<65536+2520;++caseIndex)for(unsigned alias=0;alias<2;++alias){
  ...
  prepare(phase);u8*obj=(u8*)fixture.objects[0];u8*source=(u8*)fixture.objects[alias?0:1];obj[0x2b]=queue;
  ...
  assert(oracle(0x801e8394,(u32)(uintptr_t)source,slotArg,maskArg,anim));
  ...
  func_801E8394(source,slotArg,maskArg,anim);
  if(memcmp(&fixture,&expected,sizeof(fixture))||memcmp(&call,&expectedCall,sizeof(call))||D_801E863C!=expectedMask||D_801E86B0!=expectedSlot){
   fprintf(stderr,"CROSS CLIP FAIL phase=%u alias=%u slot=%u queue=%u anim=%d\n",phase,alias,slot,queue,anim);return 1;
  }++cases;
 }
 printf("CROSS CLIP PASS %u memory/global/call comparisons; actual retail selector and binder, controlled VM\n",cases);
}
#endif
