/* Opcode CE checked against the pinned retail dispatcher, including aliasing. */
...
static void run(unsigned value,unsigned gate,unsigned variant,unsigned seed){
 ...
 c.gpr[4]=(uintptr_t)f.sprite;c.gpr[5]=0xa50000ce;c.gpr[6]=(uintptr_t)op;c.gpr[29]=0x801fff00;c.gpr[31]=0xfffffffc;
 if(PcPortMipsRun(&c,0x8001fbe4,0xfffffffc,1000)!=PC_PORT_MIPS_HALTED){fprintf(stderr,"oracle %s pc%x\n",c.error,c.pc);exit(2);}
 expected=f;f=initial;func_8001FBE4(f.sprite,0xa50000ce,op);
 if(memcmp(&f,&expected,sizeof(f))){fprintf(stderr,"SPRITE CE FAIL case=%u value=%x gate=%u variant=%u seed=%u\n",cases,value,gate,variant,seed);exit(1);}
 cases++;
}
int main(void){
 ...
 for(unsigned value=0;value<65536;value++)for(unsigned gate=0;gate<3;gate++)for(unsigned variant=0;variant<4;variant++)for(unsigned flip=0;flip<2;flip++)run(value,gate,variant,value*17+flip*4);
 for(unsigned seed=0;seed<65536;seed++)run(0x1ff,2,0,seed);
 printf("SPRITE CE PASS cases=%u complete fixture comparison\n",cases);
}
