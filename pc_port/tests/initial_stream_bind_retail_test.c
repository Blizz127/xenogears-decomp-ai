...
extern u32 func_801DFAC4(u8*,u8*,u8*,s32,s32) __attribute__((weak));
...
int main(void){
 if(!func_801DFAC4){fputs("INITIAL STREAM BIND FAIL missing native body\n",stderr);return 1;}
 ...
 for(unsigned n=0;n<3;++n)for(unsigned flags=0;flags<4;++flags)for(unsigned limit=0;limit<3;++limit)for(unsigned gate=0;gate<9;++gate)for(unsigned pattern=0;pattern<4;++pattern)for(unsigned frame=0;frame<3;++frame)for(unsigned loop=0;loop<2;++loop)for(unsigned availability=0;availability<3;++availability)for(unsigned immediate=0;immediate<2;++immediate){
  ...
  for(unsigned i=12;i<128;++i)fixture.pose[i]=(u16)(i*7919u+0x8765u);
  ...
  if(PcPortMipsRun(&cpu,0x801dfac4,0xfffffffcu,20000)!=PC_PORT_MIPS_HALTED){fprintf(stderr,"INITIAL STREAM BIND FAIL oracle %s\n",cpu.error);return 1;}expected=fixture;u32 result=cpu.gpr[2];fixture=initial;u32 actual=func_801DFAC4((u8*)fixture.pool,root,(u8*)fixture.pose,loop+2,tag);
  if(actual!=result||memcmp(&fixture,&expected,sizeof(fixture))){fprintf(stderr,"INITIAL STREAM BIND FAIL n=%u flags=%u limit=%u gate=%u pattern=%u frame=%u loop=%u availability=%u immediate=%u result=%x/%x\n",counts[n],flags,limit,gate,pattern,frame,loop,availability,immediate,actual,result);return 1;}++cases;
 }
 printf("INITIAL STREAM BIND PASS %u cases; real pose/pool/release and full memory/return\n",cases);
}
