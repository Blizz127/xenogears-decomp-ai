...
extern int32_t ab_helper_impl(void *, int32_t);

#define GUARD(name) void name(void) { fputs("SPRITE AB FAIL unexpected callee " #name "\n", stderr); abort(); }
...
void MTC2(unsigned int value, int reg) { (void)value; (void)reg; fputs("SPRITE AB FAIL unexpected MTC2\n", stderr); abort(); }
unsigned int MFC2(int reg) { (void)reg; fputs("SPRITE AB FAIL unexpected MFC2\n", stderr); abort(); }
int doCOP2(int command) { (void)command; fputs("SPRITE AB FAIL unexpected doCOP2\n", stderr); abort(); }
...
int32_t func_80022CAC(void *sprite, int32_t value)
{
    ...
    memcpy(&native_entry_pos, (uint8_t *)sprite + 8, 4);
    return ab_helper_impl(sprite, value);
}
...
static int bridge(void *u, PcPortMipsCpu *c, uint32_t target)
{
    ...
    uint8_t *entry = address(c->gpr[4] + 8, 4); if (!entry) return -1; memcpy(&retail_entry_pos, entry, 4);
    return 0;
}
...
static void compare(uint8_t operand, int16_t scale, uint16_t timer, uint32_t position, unsigned alias)
{
    ...
    uint8_t *source = alias == 0 ? fixture.ops : (alias == 1 ? p + 0x2c : (alias == 2 ? p + 0x3a : p + 0x08));
    put32(p + 0x08, position); put16(p + 0x2c, (uint16_t)scale); put16(p + 0x3a, timer);
    ...
    c.gpr[4] = (uint32_t)(uintptr_t)p; c.gpr[5] = 0xabu; c.gpr[6] = (uint32_t)(uintptr_t)source;
    c.gpr[29] = 0x801fff00u; c.gpr[31] = 0xfffffffcu;
    if (PcPortMipsRun(&c, 0x8001fbe4u, 0xfffffffcu, 4096) != PC_PORT_MIPS_HALTED) {
        fprintf(stderr, "SPRITE AB oracle failure operand=%02x scale=%04x timer=%04x alias=%u: %s\n", operand, (uint16_t)scale, timer, alias, c.error); exit(2);
    }
    expected = fixture; fixture = initial; func_8001FBE4(p, 0xabu, source);
...
int main(void)
{
    ...
    if (getenv("SPRITE_AB_QUICK")) {
        unsigned operand = getenv("SPRITE_AB_OPERAND") ? strtoul(getenv("SPRITE_AB_OPERAND"), 0, 0) : 1;
        unsigned scale = getenv("SPRITE_AB_SCALE") ? strtoul(getenv("SPRITE_AB_SCALE"), 0, 0) : 0x1000;
        unsigned timer = getenv("SPRITE_AB_TIMER") ? strtoul(getenv("SPRITE_AB_TIMER"), 0, 0) : 0;
        unsigned alias = getenv("SPRITE_AB_ALIAS") ? strtoul(getenv("SPRITE_AB_ALIAS"), 0, 0) : 0;
        unsigned position = getenv("SPRITE_AB_POSITION") ? strtoul(getenv("SPRITE_AB_POSITION"), 0, 0) : 0x00120000;
        compare((uint8_t)operand, (int16_t)(uint16_t)scale, (uint16_t)timer, position, alias); printf("SPRITE AB QUICK PASS %u cases\n", cases); return 0;
    }
    ...
    printf("SPRITE AB PASS %u cases: full operand/scale/timer domains, signed fixed-point rounding, helper ordering, and operand aliases\n", cases); return 0;
}
