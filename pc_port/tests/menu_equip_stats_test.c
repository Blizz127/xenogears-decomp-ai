...
int main(void){
 FILE*f=fopen("disc/menu.bin","rb");if(!f)return 2;size_t n=fread(ram+0x1c5000,1,0x3b000,f);fclose(f);if(n<0x1ec2c)return 2;
 unsigned cases=0;
 for(unsigned seed=0;seed<768;seed++)for(unsigned ch=0;ch<MAX_GAME_CHARACTERS;ch++)for(unsigned special=0;special<2;special++){
  init(seed,ch,0,0);u8*c=(u8*)&g_GameState.characters[ch];
  if(seed<256)memset(c,seed,0xa4);
  c[0x56]=special?4:0;
  GameState initial=g_GameState;
  PcPortMipsBus bus={.read=rd,.write=wr};PcPortMipsCpu cpu;PcPortMipsCpuInit(&cpu,&bus);cpu.gpr[4]=0x80101000;cpu.gpr[5]=0xffffff00|ch;cpu.gpr[29]=0x801ff000;cpu.gpr[31]=0xfffffffc;
  if(PcPortMipsRun(&cpu,0x801e3a80,0xfffffffc,1000)){fprintf(stderr,"FAIL retail %s\n",cpu.error);return 1;}
  MenuUnk6 expected=resources;GameState expectedState=g_GameState;
  init(seed,ch,0,0);g_GameState=initial;func_801E3A80(&resources,ch);
  if(memcmp(&expected,&resources,sizeof resources)||memcmp(&expectedState,&g_GameState,sizeof g_GameState)){
   fprintf(stderr,"FAIL equip stats seed=%u ch=%u special=%u\n",seed,ch,special);return 1;
  }cases++;
 }
 printf("EQUIP STATS PASS cases=%u\n",cases);return 0;
}
