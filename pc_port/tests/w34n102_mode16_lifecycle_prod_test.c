/* Focused production certificate for retail mode-16 lifecycle. */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
enum EventKind {
    EV_FRAMEBUFFER = 1, EV_MOVE_IMAGE, EV_DRAW_SYNC, EV_TRANSITION,
    EV_SECOND_FINISH, EV_OBJECT_POOL, EV_CROSS_PRODUCTS, EV_CD_SYNC,
    EV_WDS_CLEANUP, EV_OBJECT_MATRIX, EV_GPU_A, EV_GPU_B, EV_THIRD_WAVE,
    EV_BSS, EV_HEAP_TABLE, EV_UPLOAD_A, EV_UPLOAD_B, EV_DRAW_PACKETS,
    EV_TABLES, EV_FIRST_WDS, EV_ARCHIVE_INDEX, EV_TERRAIN_POSITION,
    EV_CD_WORK, EV_VSYNC, EV_DISTANCE, EV_HEAP_FREE, EV_SOUND_ADD,
    EV_ARCHIVE_SIZE, EV_SONG_CREATE, EV_SONG_LEVEL, EV_REGISTER, EV_978FC,
    EV_8901C, EV_865A0, EV_75228, EV_SOUND_SHUTDOWN, EV_SOUND_RELEASE,
    EV_84818, EV_86568, EV_866C8, EV_74F04, EV_750DC, EV_88FF4, EV_89128,
    EV_97D64, EV_976A0
};
...
{
    record_event(EV_CD_SYNC, (uintptr_t)(u32)mode, 0u, 0u, 0u);
}

void func_8001B66C(void)
{
    record_event(EV_WDS_CLEANUP, 0u, 0u, 0u, 0u);
}
...
{
    record_event(EV_DISTANCE, 0u, 0u, 0u, 0u);
    return 0u;
}
...
{
    record_event(EV_SONG_LEVEL, (uintptr_t)manager, (uintptr_t)(u32)level,
...
{
    record_event(EV_REGISTER, (uintptr_t)init_cb, (uintptr_t)update_cb, 0u, 0u);
}
...
static void seed_setup(void)
{
    ...
    for (index = 0; index < 32; index++)
        bytes[index] = (uint8_t)(0x40 + index);
    memcpy(PSX_ADDR(UINT32_C(0x8009A180)), bytes, sizeof(bytes));
...
static void test_setup(void)
{
    static const int expected[] = {
        ...
        EV_WDS_CLEANUP, EV_OBJECT_MATRIX, EV_GPU_A, EV_GPU_B, EV_THIRD_WAVE,
        EV_BSS, EV_HEAP_TABLE, EV_UPLOAD_A, EV_UPLOAD_B, EV_DRAW_PACKETS,
        EV_TABLES, EV_CD_SYNC, EV_FIRST_WDS, EV_ARCHIVE_INDEX,
        EV_TERRAIN_POSITION, EV_CD_WORK, EV_VSYNC, EV_DISTANCE, EV_HEAP_FREE,
        EV_SOUND_ADD, EV_ARCHIVE_SIZE, EV_SONG_CREATE, EV_SONG_LEVEL,
        EV_REGISTER, EV_REGISTER, EV_REGISTER, EV_REGISTER, EV_REGISTER,
        EV_REGISTER, EV_REGISTER, EV_REGISTER, EV_978FC, EV_8901C, EV_865A0,
        EV_75228
    };
    static const u32 registrations[][2] = {
        {UINT32_C(0x800923A8), UINT32_C(0x800925A0)},
        {UINT32_C(0x80081174), UINT32_C(0x800811C0)},
        {UINT32_C(0x800813E8), UINT32_C(0x80081470)},
        {UINT32_C(0x800817A0), UINT32_C(0x80081868)},
        {UINT32_C(0x800819C8), UINT32_C(0x80081B24)},
        {UINT32_C(0x80081C3C), UINT32_C(0x80081D80)},
        {UINT32_C(0x80081FB4), UINT32_C(0x80081FD8)},
        {UINT32_C(0x80078948), UINT32_C(0x80078950)}
    };
    int index;

    seed_setup();
    check(wm_80080D00() == 0, "setup.return_zero");
    check_event_kinds(expected, (int)(sizeof(expected) / sizeof(expected[0])),
                      "setup.stage_order");
    check(events[1].a1 == UINT32_C(0x014000D8) && events[1].a2 == 704u &&
          events[1].a3 == 256u, "setup.move_image_args");
    ...
    check(read32(UINT32_C(0x8009CCA4)) == 2u &&
          ...
          "setup.state_values");
    check(read32(UINT32_C(0x8009C5AC)) == UINT32_C(0x04000000) &&
          read32(UINT32_C(0x8009C5B0)) == UINT32_C(0xFFF40000) &&
          read32(UINT32_C(0x8009C5B4)) == UINT32_C(0x04000000),
          "setup.position_vector");
    check(count_kind(EV_FIRST_WDS) == 1, "setup.wds_load");
    check(D_80062528 == MANAGER && count_kind(EV_SONG_CREATE) == 1 &&
          count_kind(EV_SONG_LEVEL) == 1, "setup.song_start");
    check(count_kind(EV_REGISTER) == 8, "setup.registration_count");
    for (index = 0; index < 8; index++)
        check(events[31 + index].a0 == registrations[index][0] &&
              events[31 + index].a1 == registrations[index][1],
...
static void test_teardown(void)
{
    ...
    for (index = 0; index < 5; index++)
        write32(sources[index], UINT32_C(0x800A0000) + (u32)index * 0x100u);
    write16(UINT32_C(0x8009BD3A), UINT16_C(0xBEEF));
    wm_8008106C();
    check_event_kinds(expected, (int)(sizeof(expected) / sizeof(expected[0])),
                      "teardown.call_order");
    check(events[1].a0 == (uintptr_t)PSX_ADDR(SEDS_SOURCE) &&
          events[2].a0 == (uintptr_t)PSX_ADDR(SEDS_SOURCE),
          "teardown.seds_free");
    check(read16(UINT32_C(0x8006F94E)) == 506u &&
          read16(UINT32_C(0x8006F950)) == UINT16_C(0xBEEF) &&
          read16(UINT32_C(0x8006F954)) == 0u &&
...
int main(void)
{
    ...
    test_teardown();
    if (failures != 0) {
        fprintf(stderr, "W34N102 MODE16 LIFECYCLE CERTIFICATE: %d failure(s)\n",
                failures);
        return 1;
    }
    puts("W34N102 MODE16 LIFECYCLE CERTIFICATE PASS");
    return 0;
}
