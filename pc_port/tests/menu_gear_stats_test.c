...
static int memory(u32 a,unsigned w,u32*v,int write){
 ...
 if(a>=0x801e9808&&(uint64_t)a+w<=0x801e9908)p=D_801E9808+a-0x801e9808;
...
int main(void){
 FILE*f=fopen("disc/menu.bin","rb");if(!f)return 2;size_t n=fread(ram+0x1c5000,1,0x3b000,f);fclose(f);if(n<0x1eecc)return 2;
 unsigned cases=0;
 for(unsigned seed=0;seed<768;seed++)for(unsigned id=0;id<MAX_GAME_GEARS;id++)for(unsigned flag=0;flag<2;flag++){
  init(seed,0,0,0);u8*g=(u8*)&g_GameState.gears[id];
  if(seed<256)memset(g,seed,0xa4);
  for(unsigned i=0;i<256;i++)D_801E9808[i]=(seed+i)%MAX_GAME_CHARACTERS;
  *(u16*)((u8*)&g_GameState+0x22b6)=flag?0x1000:0;
  GameState initial=g_GameState;u8 initialMap[256];memcpy(initialMap,D_801E9808,256);
  PcPortMipsBus bus={.read=rd,.write=wr};PcPortMipsCpu cpu;PcPortMipsCpuInit(&cpu,&bus);cpu.gpr[4]=0x80101000;cpu.gpr[5]=0xffffff00|id;cpu.gpr[29]=0x801ff000;cpu.gpr[31]=0xfffffffc;
  if(PcPortMipsRun(&cpu,0x801e3c2c,0xfffffffc,1000)){fprintf(stderr,"FAIL retail %s\n",cpu.error);return 1;}
  MenuUnk6 expected=resources;GameState expectedState=g_GameState;u8 expectedMap[256];memcpy(expectedMap,D_801E9808,256);
  init(seed,0,0,0);g_GameState=initial;memcpy(D_801E9808,initialMap,256);func_801E3C2C(&resources,id);
  if(memcmp(&expected,&resources,sizeof resources)||memcmp(&expectedState,&g_GameState,sizeof g_GameState)||memcmp(expectedMap,D_801E9808,256)){
   fprintf(stderr,"FAIL gear stats seed=%u id=%u flag=%u\n",seed,id,flag);return 1;
  }cases++;
 }
 printf("GEAR STATS PASS cases=%u\n",cases);return 0;
}
