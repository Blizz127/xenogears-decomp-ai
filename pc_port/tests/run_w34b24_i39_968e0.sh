...
BUILD_DIR="${W34B24_I39_BUILD_DIR:-$ROOT/pc_port/build_native/w34b24_i39}"
mkdir -p "$BUILD_DIR"
cd "$ROOT"
...
CC="${CC:-clang}"
BASE=(-std=gnu17 -DXENO_PC_PORT -DSKIP_ASM -D_LANGUAGE_C -DWM_968E0_TEST_TRACE -no-pie)
WARN=(-Wall -Wextra -Wconversion -Wsign-conversion -Werror)
INC=(-Ipc_port/include_shim -Iinclude -Ipc_port/src)
SRC=(pc_port/tests/w34b24_i39_968e0_prod_test.c
     pc_port/src/psx_memory.c
     pc_port/src/world_map_helper_968e0.c)
...
require_retail() {
    ...
    check_slice 0x26DF0 188 "27221b582c93408a01c5a64c4b05aa1fa6cb1d6a4eef07311f16237a28963c93" "0x800968E0"
}
...
b=pathlib.Path('disc/world_map.bin').read_bytes()
off=0x80070CA0-0x8006FAF0
want=[0x80096910,0x80096950,0x80096950,0x80096950,0x80096918,0x80096958]
got=[struct.unpack_from('<I', b, off+i*4)[0] for i in range(6)]
if got!=want:
    print('ERROR: 0x80070CA0 switch table', [hex(x) for x in got], file=sys.stderr)
    sys.exit(1)
print('SWITCH TABLE 0x80070CA0 closed:', [hex(x) for x in got])
PY
...
for regime in focused_O0 focused_O2 focused_ubsan; do
    rg -q '^W34B24-I39 0x800968E0 focused oracle PASS' "$BUILD_DIR/$regime.stdout"
...
mutants=(
    WM_968E0_MUTANT_NO_GUARD
    WM_968E0_MUTANT_CASE0_ONE
    WM_968E0_MUTANT_SKIP_PUBLISH
    WM_968E0_MUTANT_WRONG_RING
)
killed=0
...
echo "W34B24-I39 0x800968E0 focused certificate PASS"
