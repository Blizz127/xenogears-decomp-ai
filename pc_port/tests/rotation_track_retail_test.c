...
extern u32 FieldDecodeRotationTrack(u8*,u8*,u32,u32) __attribute__((weak));
...
int main(void){
    if(!FieldDecodeRotationTrack){fputs("ROTATION TRACK FAIL missing native stage\n",stderr);return 1;}
    ...
    for(unsigned time=0;time<8;++time)for(unsigned data=0;data<5;++data){
        ...
        *(u32*)(node+0x70)=behavior==7?0:(u32)(uintptr_t)track;
        ...
        *(u16*)(node+0x54)=data==0?17:0x8000;*(u16*)(node+0x56)=data==0?0xfff:0x7fff;*(u16*)(node+0x58)=data==0?-31:0xffff;
        if(data==4&&(flags&15)==4)memcpy(node+0x54,track+0xa,6);
        ...
        if(PcPortMipsRun(&cpu,0x801ddc4c,0x801de3ec,10000)!=PC_PORT_MIPS_HALTED){fprintf(stderr,"ROTATION TRACK FAIL oracle %s flags=%u\n",cpu.error,flags);return 1;}
        ...
        u32 result=FieldDecodeRotationTrack(node,(u8*)fixture.pool,0x23,0x80000000);
        if(result!=cpu.gpr[21]||memcmp(&fixture,&expected,sizeof(fixture))||nCalls!=expectedCount||memcmp(calls,expectedCalls,sizeof(calls))){
            fprintf(stderr,"ROTATION TRACK FAIL flags=%u behavior=%u time=%d data=%u\n",flags,behavior,times[time],data);return 1;
        }
        ++cases;
    }
    printf("ROTATION TRACK PASS %u cases: all modes, masks, lifecycle, stream, SDK boundaries\n",cases);
}
