...
BUILD_DIR="${TEMP1_GFX_FREE_BUILD_DIR:-$ROOT/pc_port/build_native/temp1_gfx_free}"
CC="${CC:-gcc}"
mkdir -p "$BUILD_DIR"
cd "$ROOT"
...
echo "retail GfxFreeWorkBuffers slice sha256: $(dd if=disc/SLUS_006.64 bs=1 \
    skip=$((0x800 + 0x80024FB8 - 0x80010000)) count=$((0x2c)) status=none \
...
build_and_run() {
    ...
    "$CC" "${BASE[@]}" "${INC[@]}" -Wall -Wextra -Werror "$@" \
        -c pc_port/tests/temp1_gfx_free_retail_test.c \
...
grep -Eq '^TEMP1 GFX FREE certificate PASS checks=[0-9]+$' "$BUILD_DIR/O0.stdout"
