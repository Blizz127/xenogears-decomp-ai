/* Differential test: host-compiled src/battle/main14.c func_8007A744
 * vs the retail bytes (asm/battle/nonmatchings/main14/func_8007A744.s)
 * executed by remu. Fails on any behavioral mismatch.
 *
 * func_8007A744(a0): ((D_800CCD64[368 * (a0 & 0xFF)] & 0xC000) == 0) gated on
 * D_800D2DCC[a0 & 0xFF] != 0 and D_800C3EB7[28 * (a0 & 0xFF)] == 0; a1..a3 unused.
 *
 ...
 *   tools/remu/run_diff.sh diff_8007A744 src/battle/main14.c
 */
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
#define RETAIL_S "asm/battle/nonmatchings/main14/func_8007A744.s"
...
#define SPAN_LEN (0x800E3BF6u - 0x800C3EB7u)
...
static int check_one(remu_t *m, uint32_t entry, uint32_t seed,
    ...
    if (span[E3_OFF + t] == 0)
        ...
        want = 0;
    else if ((h64 & 0xC000u) != 0)
        want = 0;
    
    else
        want = 1;

    uint32_t got = func_8007A744((u8)a0);
...
int main(void) {
    ...
    printf("DIFF 8007A744 OK (%d seeds, retail-exact)\n", 256 * 4 * 10);
    return 0;
}
