/* Differential test: host-compiled src/battle/main17.c func_8007AA1C
 * vs the retail bytes (asm/battle/nonmatchings/main17/func_8007AA1C.s)
 * executed by remu. Fails on any behavioral mismatch.
 *
 * Build (from repo root):
 *   gcc -std=gnu17 -O2 -Iinclude -Isrc -Itools/remu \
 *       tools/remu/tests/diff_8007AA1C.c build-main17.o tools/remu/remu.c \
 *       -o /tmp/diff_8007AA1C && /tmp/diff_8007AA1C
...
#define RETAIL_S "asm/battle/nonmatchings/main17/func_8007AA1C.s"
#define RETAIL_ENTRY 0x8007AA1Cu
...
static int check_one(remu_t *m, uint32_t entry, uint32_t seed) {
    ...
    /* expected v0 from the pre-state (retail leaves the difference in v0) */
    uint32_t want_v0 = (uint32_t)((int32_t)init - (int32_t)operand);
...
int main(void) {
    ...
    printf("DIFF 8007AA1C OK (%d seeds, retail-exact)\n", n);
    remu_destroy(m);
    return 0;
}
