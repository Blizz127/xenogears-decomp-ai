/* Differential test: host-compiled src/battle/main125.c func_800BEF24
 * vs the retail bytes (asm/battle/nonmatchings/main125/func_800BEF24.s)
 * executed by remu. Fails on any behavioral mismatch.
 *
 * func_800BEF24(a, b): pack two s16 pairs into words
 *   w1 = u16(a[2]) | u16(a[0xA]) << 16, w0 = u16(b[2]) | u16(b[0xA]) << 16,
 * call func_80023124(w0, w1), return (s16)result sign-extended.
 * (The lh/sh/lhu round-trips through the stack are a plain & 0xFFFF.)
 *
 ...
 *   tools/remu/run_diff.sh diff_800BEF24 src/battle/main125.c
 */
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
#define RETAIL_S "asm/battle/nonmatchings/main125/func_800BEF24.s"
#define RA 0x801F0000u
#define RB 0x801F0010u
#define SLEN 16
...
static int check_one(remu_t *m, uint32_t entry, uint32_t seed) {
    uint32_t x = seed ^ 0xBEF24000u;
    uint8_t sa[SLEN], sb[SLEN];
    static uint8_t h_a[SLEN], h_b[SLEN];

    for (int i = 0; i < SLEN; i++) {
        sa[i] = (uint8_t)(lcg(&x) >> 16);
        sb[i] = (uint8_t)(lcg(&x) >> 16);
    }
    /* expected packed words (u16 of the signed halfwords) */
    uint32_t w1 = (uint16_t)rd16(sa + 2) |
                  ((uint32_t)(uint16_t)rd16(sa + 0xA) << 16);
    uint32_t w0 = (uint16_t)rd16(sb + 2) |
                  ((uint32_t)(uint16_t)rd16(sb + 0xA) << 16);

    /* host side */
    memcpy(h_a, sa, SLEN);
    memcpy(h_b, sb, SLEN);
    g_nc = 0;
    u32 got = func_800BEF24(h_a, h_b);
    ...
    if (memcmp(h_a, sa, SLEN) != 0 || memcmp(h_b, sb, SLEN) != 0) {
        printf("FAIL seed=%08X host clobbered inputs\n", seed);
        return 0;
    }

    /* retail side */
    if (remu_poke(m, RA, sa, SLEN)) { printf("FAIL poke a\n"); return 0; }
    if (remu_poke(m, RB, sb, SLEN)) { printf("FAIL poke b\n"); return 0; }
    int rc = remu_call(m, entry, RA, RB, 0, 0);
    if (rc != 0) {
    ...
    uint8_t ra[SLEN], rb[SLEN];
    if (remu_peek(m, RA, ra, SLEN)) { printf("FAIL peek a\n"); return 0; }
    if (remu_peek(m, RB, rb, SLEN)) { printf("FAIL peek b\n"); return 0; }
    if (memcmp(ra, sa, SLEN) != 0 || memcmp(rb, sb, SLEN) != 0) {
        printf("FAIL seed=%08X retail clobbered inputs\n", seed);
        return 0;
    }
    return 1;
}
...
int main(void) {
    ...
    if (entry != 0x800BEF24u) { printf("FAIL load entry=%08X\n", entry); return 1; }
    ...
    uint32_t s = 0x3456789Au;
    for (int i = 0; i < 200; i++) {
        s = s * 1103515245u + 12345u;
        if (!check_one(m, entry, s)) return 1;
        n++;
    }
    printf("DIFF 800BEF24 OK (%d seeds, retail-exact)\n", n);
    remu_destroy(m);
    return 0;
}
