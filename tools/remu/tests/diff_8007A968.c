/* Differential test: host-compiled src/battle/main16.c func_8007A968
 * vs the retail bytes (asm/battle/nonmatchings/main16/func_8007A968.s)
 ...
 *   tools/remu/run_diff.sh diff_8007A968 src/battle/main16.c
 */
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
#define RETAIL_S "asm/battle/nonmatchings/main16/func_8007A968.s"
#define TAB_BASE 0x800D3410u
/* 256x64 rows plus slack: retail stores at row + (p[1] << 2) with p[1] up
 * to 0xFF, so the max touch is (0xFF << 6) + 0x3FC + 4 = 17344. */
...
static int check_one(remu_t *m, uint32_t entry, uint32_t seed) {
    ...
    /* host side */
    memcpy(D_800D3410, tab, TAB_LEN);
    memcpy(h_board, board, sizeof(board));
    u8 *h_pp = h_board;
    func_8007A968(&h_pp, idx);
    ...
    uint32_t want_off = ((uint32_t)idx << 6) + ((uint32_t)b1 << 2);
...
int main(void) {
    ...
    if (entry != 0x8007A968u) { printf("FAIL load entry=%08X\n", entry); return 1; }
    ...
    printf("DIFF 8007A968 OK (%d seeds, retail-exact)\n", n);
    remu_destroy(m);
    return 0;
}
