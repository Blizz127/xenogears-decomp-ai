/* Differential test: host-compiled src/battle/main17.c func_8007AA60
 * vs the retail bytes (asm/battle/nonmatchings/main17/func_8007AA60.s)
 * executed by remu. Fails on any behavioral mismatch.
 *
 * Build (from repo root):
 *   gcc -std=gnu17 -O2 -Iinclude -Isrc -Itools/remu \
 *       tools/remu/tests/diff_8007AA60.c build-main17.o tools/remu/remu.c \
 *       -o /tmp/diff_8007AA60 && /tmp/diff_8007AA60
...
#define RETAIL_S "asm/battle/nonmatchings/main17/func_8007AA60.s"
#define RETAIL_ENTRY 0x8007AA60u
...
static int check_one(remu_t *m, uint32_t entry, uint32_t seed) {
     ...
     * the compared value is the sign-extended low half of the product) */
    uint32_t pre_prod = (uint32_t)init * (uint32_t)operand;
    int32_t pre_lo = (int32_t)(int16_t)(uint16_t)pre_prod;
    uint32_t want_v0 = (pre_lo < 0x100) ? 1u : 0u;
...
int main(void) {
    ...
    printf("DIFF 8007AA60 OK (%d seeds, retail-exact)\n", n);
    remu_destroy(m);
    return 0;
}
