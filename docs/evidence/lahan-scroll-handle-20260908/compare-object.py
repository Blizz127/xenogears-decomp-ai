...
sym=next(s for s in symbols if string(strings,s[0])=='func_801D1464')
start,size,section=sym[1],sym[2],sym[5]
out=bytearray(data(sections[section])[start:start+size]);relocs=[]
addresses={'func_801CE198':0x801CE198,'func_801D22F4':0x801D22F4,'func_801E8018':0x801E8018,'HeapAlloc':0x80031BDC,'bzero':0x8003F8E8,'func_801C72BC':0x801C72BC,'g_Menu':0x800625A0,'D_801EA548':0x801EA548}
for sec in sections:
...
assert len(relocs)==3
expected=Path('disc/menu.bin').read_bytes()[0xc464:0xc4b0]
assert hashlib.sha256(expected).hexdigest()=='2c70abd9371739401adb9cb7c844615fbe75d09905b76159ed4b44107b5385fe'
...
print(json.dumps({'function':'func_801D1464','object_sha256':hashlib.sha256(b).hexdigest(),'compiled_bytes':size,'retail_bytes':len(expected),'relocations':relocs,'differences':differences,'match':out==expected,'scope':'Object function with zero-addend relocations resolved to retail addresses; not whole-ROM match'},indent=2))
assert out==expected
