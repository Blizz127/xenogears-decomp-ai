...
p=Path('build/src/field/main/misc8.c.o');b=p.read_bytes()
...
for i in range(0,sy[5],sy[9]):
 ...
 if string(strings,r[0])=='func_8008399C': break
else: raise AssertionError('function absent')
print(json.dumps({'object':str(p),'object_sha256':hashlib.sha256(b).hexdigest(),'compiled_bytes':r[2],'retail_bytes':1980,'exact_match':False if r[2]!=1980 else 'NOT_CHECKED','scope':'Size gate only; relocation-normalized byte comparison not run if sizes differ.'},indent=2))
assert r[2]==1980, 'compiled function size differs from retail'
